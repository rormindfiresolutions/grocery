class OrderLinesController < ApplicationController
	...
	def create
		...
		current_user.user_baskets.each do |product|
			...
			# To check if a Shop has a certain quantity required by Customer before Ordering
	...
	private
		...
		# To check if User ordering without their Personal Information
		def check_user_profile
			if current_user.user_profile.nil?
				flash[:error] = 'Please provide your Personal Information'
				redirect_to new_user_profile_path
			end
		end

		# To check if User ordering without their Address to deliver
		def check_address
